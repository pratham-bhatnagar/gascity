...
type GateResult struct {
	// Due is true if the gate condition is satisfied and the order should run.
	Due bool
...
// CheckGate evaluates an order's gate condition and returns whether it's due.
...
// non-event gates.
func CheckGate(a Order, now time.Time, lastRunFn LastRunFunc, ep events.Provider, cursorFn CursorFunc) GateResult {
	switch a.Gate {
	case "cooldown":
	...
	case "manual":
		return GateResult{Due: false, Reason: "manual gate — use gc order run"}
	default:
...
func checkCooldown(a Order, now time.Time, lastRunFn LastRunFunc) GateResult {
...
func checkCron(a Order, now time.Time, lastRunFn LastRunFunc) GateResult {
...
func checkCondition(a Order) GateResult {
...
func checkEvent(a Order, ep events.Provider, cursorFn CursorFunc) GateResult {
	if ep == nil {
