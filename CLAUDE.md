...
**YOU ARE IN: `gastown/polecats/slit/`** — This is YOUR worktree. Stay here.
...
**Your mail address:** `gastown/polecats/slit`
**Your rig:** gastown
...
| Ask Witness for help | `gt mail send gastown/witness -s "HELP" -m "..."` | ~~gt nudge witness~~ |
...
```bash
gt mail send gastown/witness -s "HELP: <problem>" -m "Issue: ...
Problem: ...
Tried: ...
Question: ..."
```
...
## DI MCP Requirements (MANDATORY)

Every polecat session MUST complete two DI MCP steps before exiting:

### 1. Save Session Learnings (memory_remember)

Before pushing, call `memory_remember` via DI MCP to persist at least one
learning from your session. Categories:
- **pattern** — reusable approach that worked
- **anti-pattern** — approach that failed
- **decision** — non-obvious choice and rationale
- **incident** — unexpected issue and resolution
...
memory_remember(
    ...
    scope="rig",
    confidence=0.8,
    source_bead="<issue-id>"
)
```

**Fallback:** If DI MCP is unavailable, persist learnings to bead notes:
```bash
bd update <issue-id> --notes "SESSION LEARNINGS: <what you learned>"
```

### 2. Session Report (wasteland_stamp + bead notes)

Call `wasteland_stamp(completion_id="<issue-id>")` to score your completion,
then persist a session summary to bead notes:
```bash
bd update <issue-id> --notes "SESSION REPORT:
Summary: <what was implemented>
Commits: <count>
Self-grade: <A-F>
Memories saved: <memory IDs>"
```

**Fallback:** If wasteland_stamp fails, bead notes report alone is sufficient.

These steps are enforced in the `mol-polecat-work` formula (steps
`save-session-learnings` and `session-report`).
...
- Skip DI MCP memory save or session report
